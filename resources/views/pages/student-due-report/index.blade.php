                ...
                @if($availableCategories->isNotEmpty())
                    <div class="fees-report-category-card mt-3 p-3" style="border:1px solid #e5e7eb;border-radius:12px;">
                        <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap" style="gap:.75rem;">
                            <div><p class="mb-0 font-weight-bold">PDF Column Selection</p><small class="text-muted">Choose which fee categories should appear in the student due report and PDF.</small></div>
                            <div class="form-check mb-0"><input class="form-check-input" type="checkbox" id="student-due-toggle-all" {{ count($selectedCategoryKeys) === count($availableCategories) ? 'checked' : '' }}><label class="form-check-label font-weight-bold" for="student-due-toggle-all">Select all</label></div>
                        </div>
                        <div class="row">
...
<script>
document.addEventListener('DOMContentLoaded', function () {
    const form = document.querySelector('form[action*="student-due-report"]');
    ...
    const toggle = document.getElementById('student-due-toggle-all');
    const checks = Array.from(document.querySelectorAll('.student-due-category-checkbox'));
    if (!toggle) return;
