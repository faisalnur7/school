...
<script>
    ...
    const savedExpenseMethod = '{{ old('payment_method', $expense->payment_method) }}';
    const savedExpenseAccount = '{{ old('account_id', $expense->account_id ?? '') }}';
    ...
    $(document).ready(function () {        const savedAccountType = '{{ old('account_type', $expense->account_type ?? '') }}';

        if (savedAccountType) {
            $('#expenseAccountType').val(savedAccountType);
        }
        if (expenseMethodTypeMap[savedExpenseMethod]) {
            expenseLoadAccounts(savedExpenseMethod, savedExpenseAccount);
        }
