...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h4 class="mb-0 font-weight-bold"><i class="fas fa-chart-bar text-info mr-2"></i>Marks Preview</h4>
    ...
    <div class="row">
        ...
        <div class="col-md-10">
            ...
            <div class="card mb-3">
                <div class="card-body py-2">
                    ...
                    <span class="ml-3 text-muted">
                        Showing <strong>{{ $marks->count() }}</strong> &mdash; Pass Mark: <strong>{{ $passMark }}</strong>
                    </span>
                </div>
            </div>
            ...
            {{-- Stats --}}
            @php
                $allM        = \App\Models\ExamMark::where('exam_id', $exam->id)->where('subject_id', $subject->id)
                                ->whereIn('student_id', \App\Models\Student::where('status',1)
                                    ->whereHas('academicInformations', fn($q) => $q->where('school_class_id', $classId)
                                        ->when($exam->academic_session_id, fn($q2) => $q2->where('academic_session_id', $exam->academic_session_id)))
                                    ->pluck('id'))->get();
            ...
            <div class="row mb-3">
                @foreach([
                    ['Total',   $allM->count(),              'primary', 'users'],
                    ['Passed',  $passedCount,                'success', 'check'],
                    ['Failed',  $failedCount,                'danger',  'times'],
                    ['Absent',  $absentCount,                'warning', 'user-slash'],
                    ['Average', number_format($avgMarks,1),  'info',    'chart-line'],
                    ['Highest', number_format($highest,1),   'teal',    'star'],
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <span class="badge badge-light ml-2">Full: {{ $subject->getEffectiveMarksForClass($classId)['total_marks'] }}</span>
                </div>
                <div class="card-body p-0">
                    <table class="table table-sm table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $mark->is_absent ? 'table-secondary' : ($mark->total < $passMark ? 'table-danger' : '') }}">
                                <td>{{ $i + 1 }}</td>
                                <td>
                                    <strong>{{ $mark->student->full_name_en }}</strong>
                                    @if($mark->student->full_name_bn)
                                    <br><small class="text-muted">{{ $mark->student->full_name_bn }}</small>
                                    @endif
                                </td>
                                ...
                                <td class="text-center">
                                    <span class="badge badge-{{ $mark->is_absent ? 'secondary' : ($mark->letter_grade === 'F' ? 'danger' : 'success') }} badge-pill">
                            ...
                            @empty
                            <tr><td colspan="6" class="text-center text-muted py-4">No marks entered yet.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <div class="card card-body text-center text-muted py-5">
                <i class="fas fa-book fa-3x mb-3"></i><p>Select a subject from the left.</p>
            </div>
            @endif
        </div>
    </div>
    @else
    <div class="card card-body text-center text-muted py-5">
        <i class="fas fa-th-large fa-3x mb-3"></i><p>Select a class above to preview marks.</p>
    </div>
    @endif
</div>
@endsection
