...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h4 class="mb-0 font-weight-bold"><i class="fas fa-trophy text-warning mr-2"></i>Terminal Result</h4>
            <small class="text-muted">
                {{ $exam->name }} &mdash; {{ $exam->academicSession->name_en ?? $exam->academicSession->name_bn ?? '' }}
            </small>
        </div>
        <div>
            @if($classId)
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-3">
            <div class="info-box bg-success">
                ...
                <div class="info-box-content"><span class="info-box-text">Passed</span><span class="info-box-number">{{ $passedCount }}</span></div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="info-box bg-danger">
                <span class="info-box-icon"><i class="fas fa-times-circle"></i></span>
                <div class="info-box-content"><span class="info-box-text">Failed</span><span class="info-box-number">{{ $failedCount }}</span></div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="info-box bg-info">
                <span class="info-box-icon"><i class="fas fa-star"></i></span>
                <div class="info-box-content"><span class="info-box-text">Class Avg GPA</span><span class="info-box-number">{{ $avgGpa }}</span></div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-3">
        <div class="card-body py-2 d-flex align-items-center flex-wrap">
            ...
            <div class="d-flex flex-wrap">
                ...
                <span class="badge badge-{{ $g['letter'] === 'F' ? 'danger' : ($g['gpa'] >= 4 ? 'success' : ($g['gpa'] >= 3 ? 'primary' : ($g['gpa'] >= 2 ? 'info' : 'warning'))) }} mr-1 p-1" style="font-size:10px">
                ...
                <table class="table table-bordered table-sm mb-0">
                    <thead class="thead-dark">
                        ...
                        <tr>
                            ...
                            <th class="text-center text-warning" style="font-size:10px">/{{ $cfg['total_marks'] }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="{{ $row['has_failed'] ? 'table-danger' : '' }}">
                            <td class="text-center font-weight-bold">
                                @if($row['rank'] <= 3)
                                <span class="badge badge-{{ $row['rank'] == 1 ? 'warning' : ($row['rank'] == 2 ? 'secondary' : 'info') }}">
                            ...
                            <td class="text-center {{ $sr && !($sr['passed'] ?? true) ? 'bg-danger text-white' : '' }}" style="font-size:12px">
                                @if($sr)
                            ...
                            @endforeach
                            <td class="text-center font-weight-bold">{{ number_format($row['total_obtained'], 0) }}</td>
                            <td class="text-center">{{ $row['percentage'] }}%</td>
                            <td class="text-center font-weight-bold">{{ $row['gpa'] }}</td>
                            <td class="text-center">
                                <span class="badge badge-{{ $row['gpa_label'] === 'F' ? 'danger' : ($row['gpa'] >= 4 ? 'success' : 'primary') }} badge-pill">
                                    {{ $row['gpa_label'] }}
                                </span>
                            </td>
                            <td class="text-center">
                        ...
                        @empty
                        <tr><td colspan="{{ 7 + count($subjects) }}" class="text-center text-muted py-4">No results found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
