...
document.getElementById('rep_class_id').addEventListener('change', function () {
    ...
    fetch(`/ajax/sections-by-class?class_id=${classId}`)
        ...
        .then(data => {
            data.forEach(s => {
                repSectionSelect.insertAdjacentHTML('beforeend', `<option value="${s.id}">${s.name_en}</option>`);
            });
        });
});
...
document.getElementById('btnLoadReport').addEventListener('click', function () {
    ...
    fetch(`/teacher/attendance/report/monthly/load?${qs}`, {headers: {'X-Requested-With': 'XMLHttpRequest'}})
        .then(async r => {
        ...
        .then(res => {
            wrap.innerHTML = res.ok ? res.html : `<div class="alert alert-danger mb-0">${res.msg}</div>`;
