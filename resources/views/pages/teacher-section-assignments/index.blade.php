...
<div class="container-fluid">
    ...
    <div class="card card-outline card-primary mb-3">
        <div class="card-body py-2">
            <form method="POST" action="{{ route('teacher-section-assignments.store') }}" class="form-inline flex-wrap">
                @csrf
                <select name="user_id" class="form-control form-control-sm mr-2 mb-1" required>
                    <option value="">Select Teacher</option>
                    @foreach($teachers as $t)
                    <option value="{{ $t->id }}">
                        {{ $t->name }}@if($t->email) ({{ $t->email }})@endif
                    </option>
                    @endforeach
                </select>
                ...
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <form method="POST" action="{{ route('teacher-section-assignments.destroy', $a) }}" onsubmit="return confirm('Remove this assignment?')">
                                    @csrf
                                    @method('DELETE')
