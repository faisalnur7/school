...
<div class="container-fluid">
    <div class="card">
        <div class="card-header text-white rounded-top d-flex justify-content-between align-items-center shadow p-3">
            <div>
                <h3 class="card-title mb-0 text-white text-lg">{{ __('Audit Trail') }}</h3>
                <div class="text-white-50 small">{{ __('Action history with user, date, and time') }}</div>
            </div>
            <span class="badge badge-light text-dark px-3 py-2">{{ localized_number($auditTrails->total()) }} {{ __('records') }}</span>
        </div>

        <div class="card-body px-0 pb-4 pt-0">
            @if ($auditTrails->isEmpty())
                <div class="text-center text-muted py-5">{{ __('No audit trail entries found.') }}</div>
            @else
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('Important Description') }}</th>
                                <th>{{ __('Username') }}</th>
                                <th>{{ __('Date') }}</th>
                                <th>{{ __('Time') }}</th>
                                <th>{{ __('Method') }}</th>
