...
{{-- Page Header --}}
<div class="relative mb-10 overflow-hidden rounded-2xl bg-gradient-to-br from-amber-600 via-orange-600 to-red-500 px-6 py-10 text-white shadow-2xl lg:px-10 lg:py-14">
    ...
    <div class="relative z-10">
        ...
        <h1 class="mt-4 text-3xl font-bold lg:text-4xl">Staff Members</h1>
        <p class="mt-2 text-base text-amber-100 lg:text-lg">Our administrative and support team</p>
    </div>
</div>
...
<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
    @forelse($staff as $member)
        ...
        @endphp
        <a href="{{ route('website.staff.show', $member) }}" class="group relative overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 text-center shadow-md transition-all duration-300 hover:-translate-y-1 hover:border-amber-200 hover:shadow-xl">
            ...
            <div class="relative">
                <div class="relative mx-auto mb-4 inline-block">
                    @if($member->photo_url)
                        <img src="{{ $member->photo_url }}" alt="{{ $member->name }}" class="h-24 w-24 rounded-full border-4 border-white object-cover shadow-md transition-transform duration-300 group-hover:scale-110">
                    @else
                        <div class="flex h-24 w-24 items-center justify-center rounded-full border-4 border-white bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-md">
                ...
                <p class="mt-1 text-sm font-semibold text-amber-600">{{ $desigName }}</p>
                @if($deptName)
                    <p class="mt-1 text-xs text-slate-500">{{ $deptName }}</p>
                @endif
                <div class="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-amber-600 opacity-0 transition-all duration-300 group-hover:opacity-100">
                    View Profile
    ...
    @empty
        <div class="col-span-full flex flex-col items-center justify-center rounded-2xl border border-slate-100 bg-white px-6 py-16 text-center shadow-lg">
            ...
            <p class="text-lg font-medium text-slate-600">No staff members found.</p>
