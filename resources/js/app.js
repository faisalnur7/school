...
function initChatWidgets() {
    document.querySelectorAll('[data-chat-widget-root]').forEach((widget) => {
        if (widget.dataset.chatWidgetInitialized === 'true') {
            return;
        }

        widget.dataset.chatWidgetInitialized = 'true';

        const panel = widget.querySelector('[data-chat-widget-panel]');
        const overlay = widget.querySelector('[data-chat-widget-overlay]');
        const openButtons = widget.querySelectorAll('[data-chat-widget-toggle]');
        const closeButton = widget.querySelector('[data-chat-widget-close]');
        const threadEl = widget.querySelector('[data-chat-thread]');
        const titleEl = widget.querySelector('[data-chat-title]');
        const subtitleEl = widget.querySelector('[data-chat-subtitle]');
        const avatarEl = widget.querySelector('[data-chat-avatar]');
        const unreadBadge = widget.querySelector('[data-chat-unread-badge]');
        let conversationButtons = Array.from(widget.querySelectorAll('[data-chat-conversation-trigger]'));
        const form = widget.querySelector('[data-chat-message-form]');
        const bodyInput = widget.querySelector('[data-chat-message-input]');
        const fileInput = widget.querySelector('[data-chat-attachment-input]');
        const fileLabel = widget.querySelector('[data-chat-attachment-label]');
        const attachmentPreview = widget.querySelector('[data-chat-attachment-preview]');
        const attachmentPreviewImage = widget.querySelector('[data-chat-attachment-preview-image]');
        const attachmentPreviewLabel = widget.querySelector('[data-chat-attachment-preview-label]');
        const composeEditBar = widget.querySelector('[data-chat-compose-edit-bar]');
        const composeEditCancel = widget.querySelector('[data-chat-compose-edit-cancel]');
        const threadEmptyState = threadEl ? threadEl.dataset.emptyStateHtml || '' : '';
        const currentUserId = widget.dataset.currentUserId || '';
        const messageRouteBase = widget.dataset.messageRouteBase || '';
        const csrfToken = getCsrfToken();
        let activeConversationId = widget.dataset.activeConversationId || '';
        ...
        function setConversationButtonsActive(conversationId) {
            conversationButtons.forEach((button) => {
                const isActive = button.dataset.chatConversationId === String(conversationId);
                button.classList.toggle('is-active', isActive);
                button.setAttribute('aria-current', isActive ? 'true' : 'false');
                const badge = button.querySelector('[data-chat-conversation-unread]');
                if (badge && isActive) {
                    badge.textContent = '0';
        ...
        function syncConversationButton(conversation) {
            if (!conversation) {
                return;
            }

            const scroll = widget.querySelector('[data-chat-widget-panel] .chat-widget-conversations__scroll');
            const existingButton = conversationButtons.find((button) => String(button.dataset.chatConversationId) === String(conversation.id));
            if (!existingButton) {
                return;
            }

            existingButton.classList.toggle('is-active', true);
            existingButton.setAttribute('aria-current', 'true');
            existingButton.dataset.chatConversationUrl = `/communications/conversations/${encodeURIComponent(conversation.id)}`;
            existingButton.dataset.chatConversationId = String(conversation.id);
            existingButton.innerHTML = renderWidgetConversationButton(conversation, true).trim();
            scroll?.prepend(existingButton);
            conversationButtons = Array.from(widget.querySelectorAll('[data-chat-conversation-trigger]'));
            setConversationButtonsActive(conversation.id);
        }
        ...
        function setHeader(conversation) {
            ...
            if (subtitleEl) {
                subtitleEl.textContent = conversation.subtitle || 'Conversation';
            }
            ...
            if (unreadBadge) {
                unreadBadge.textContent = conversation.unread_count > 0 ? conversation.unread_count : '';
                unreadBadge.classList.toggle('d-none', !(conversation.unread_count > 0));
            }

            if (form && messageRouteBase && conversation.id) {
                form.action = `${messageRouteBase}/${conversation.id}/messages`;
                form.dataset.conversationId = String(conversation.id);
            }

            widget.dataset.activeConversationId = String(conversation.id);
        ...
        function syncComposerEditState() {
            ...
            if (form && isEditing) {
                form.dataset.chatEditingMessageId = editingMessageId;
            } else if (form) {
                delete form.dataset.chatEditingMessageId;
            }

            composeEditBar?.classList.toggle('d-none', !isEditing);

            const submitLabel = form?.querySelector('[data-chat-submit-label]');
            if (submitLabel) {
        ...
        closeChatMessageMenus(threadEl || widget);
            bodyInput.value = getMessageBodyFromNode(messageNode);
            resizeChatComposerTextarea(bodyInput);
        ...
        function appendMessage(message) {
            appendChatMessage(threadEl, message);
        }

        async function loadConversation(url, buttonEl) {
            if (!url) {
                return;
            }
            ...
            try {
                ...
                renderChatThread(threadEl, payload.messages || [], threadEmptyState);
        ...
        async function submitMessage(event) {
            ...
            const editingId = String(form.dataset.chatEditingMessageId || '').trim();
            const isEditing = Boolean(editingId);
            const endpoint = isEditing ? getMessageUpdateEndpoint(editingId) : form.action;
            const method = isEditing ? 'PATCH' : 'POST';

            if (!bodyValue && !(fileInput && fileInput.files && fileInput.files.length)) {
                bodyInput?.focus();
                return;
            }
            ...
            try {
                ...
                if (payload.message) {
                    if (isEditing) {
                        updateChatMessageNode(threadEl, payload.message, threadEmptyState);
                    } else {
                ...
                if (payload.conversation) {
                    setHeader(payload.conversation);
                    syncConversationButton(payload.conversation);
                }

                resetChatComposer(form, bodyInput, fileInput, fileLabel, attachmentPreview, attachmentPreviewImage, attachmentPreviewLabel);
                cancelChatMessageEdit({ focus: false });
                bodyInput?.focus();
            } catch (error) {
                console.error(error);
        ...
        async function deleteMessage(messageNode) {
            ...
            try {
                ...
                removeChatMessageNode(threadEl, messageId, threadEmptyState);
                if (payload?.conversation) {
                    setHeader(payload.conversation);
                    syncConversationButton(payload.conversation);
                }
        ...
        conversationButtons.forEach((button) => {
            button.addEventListener('click', (event) => {
                event.preventDefault();
                const url = button.dataset.chatConversationUrl;
                const conversationId = button.dataset.chatConversationId;
                if (conversationId) {
                    setConversationButtonsActive(conversationId);
                }
                loadConversation(url, button);
            });
        });

        form?.addEventListener('submit', submitMessage);
        bindComposerEnterToSend(form, bodyInput);

        if (bodyInput) {
            resizeChatComposerTextarea(bodyInput);
            bodyInput.addEventListener('input', () => resizeChatComposerTextarea(bodyInput));
            bodyInput.addEventListener('change', () => resizeChatComposerTextarea(bodyInput));
        }

        fileInput?.addEventListener('change', () => {
            const file = fileInput.files?.[0];
            if (fileLabel) {
                fileLabel.textContent = file ? file.name : 'Attach image';
            }

            updateChatAttachmentPreview(file, attachmentPreview, attachmentPreviewImage, attachmentPreviewLabel);
        });
        ...
        if (widget.dataset.openOnLoad === 'true') {
            openWidget();
        }
    });
}
