...
{
    public function convert(AdmissionApplication $application, ?int $userId = null, ?int $sectionId = null): AdmissionConversion
    {
        return DB::transaction(function () use ($application, $userId, $sectionId) {
            ...
            $roll = $this->nextRoll($application->academic_session_id, $application->school_class_id, $sectionId, $data['group_id'] ?? null);
            StudentAcademicInformation::create(['student_id' => $student->id, 'academic_session_id' => $application->academic_session_id, 'school_class_id' => $application->school_class_id, 'section_id' => $sectionId, 'group_id' => $data['group_id'] ?? null, 'roll' => $roll, 'academic_status' => 'active', 'promotion_status' => 'new_admission', 'is_current' => true]);
